...
class XSwitchHorizontal : CompoundButton {
    ...
    constructor(
        ...
        val typedArray = context!!.obtainStyledAttributes(attrs, R.styleable.XSwitchHorizontal, defStyleAttr, defStyleRes)
        if (Build.VERSION.SDK_INT >= VERSION_CODES.Q) {
            saveAttributeDataForStyleable(context, R.styleable.XSwitchHorizontal, attrs, typedArray, defStyleAttr, defStyleRes)
        }
        if(typedArray.hasValue(R.styleable.XSwitchHorizontal_android_thumb)) {
            mThumbDrawable = typedArray.getDrawable(R.styleable.XSwitchHorizontal_android_thumb)
        }else{
            mThumbDrawable = Util.getDrawable(context, R.drawable.switch_thumb)
        }
        mThumbDrawable?.callback = this
        if (typedArray.hasValue(R.styleable.XSwitchHorizontal_android_track)) {
            mTrackDrawable = typedArray.getDrawable(R.styleable.XSwitchHorizontal_android_track)
        }else{
            mTrackDrawable = Util.getDrawable(context, R.drawable.switch_track)
        }
        mTrackDrawable?.callback = this

        mTextOn = typedArray.getText(R.styleable.XSwitchHorizontal_android_textOn)
        mTextOff = typedArray.getText(R.styleable.XSwitchHorizontal_android_textOff)
        mShowText = typedArray.getBoolean(R.styleable.XSwitchHorizontal_android_showText, true)
        mThumbTextPadding = typedArray.getDimensionPixelSize(R.styleable.XSwitchHorizontal_android_thumbTextPadding, 0)

        mSwitchMinWidth = typedArray.getDimensionPixelSize(R.styleable.XSwitchHorizontal_android_switchMinWidth, 0)
        mSwitchPadding = typedArray.getDimensionPixelSize(R.styleable.XSwitchHorizontal_android_switchPadding, 0)
...
//        )
        ...
        val switchWidth = Math.max(
            mSwitchMinWidth,
            mThumbWidth + mTextMaxWidth + paddingLeft + paddingRight
        )
    ...
    private fun hitThumb(x: Float, y: Float): Boolean {
        ...
        val thumbLeft = mSwitchLeft + thumbOffset - mTouchSlop
...
//            if (mTextOff == null) resources.getString(R.string.capital_off) else mTextOff!!
    ...
    override fun draw(c: Canvas?) {
        ...
        var thumbInitialLeft = switchLeft + getThumbOffset()
        ...
        if (mThumbDrawable != null) {
            mThumbDrawable!!.getPadding(padding)
            thumbLeft = thumbInitialLeft - padding.left
            thumbRight = thumbInitialLeft + mThumbWidth + padding.right
            mThumbDrawable!!.setBounds(thumbLeft, switchTop, thumbRight, switchBottom)
            val background = background
            background?.setHotspotBounds(thumbLeft, switchTop, thumbRight, switchBottom)
        }
        if (thumbLeft - trackLeft > trackRight - thumbRight){ // text on left
            mTextBoundLeft = trackLeft.toFloat()
            mTextBoundRight = thumbLeft.toFloat()
        }else{ // text on right
            mTextBoundLeft = thumbRight.toFloat()
            mTextBoundRight = trackRight.toFloat()
        }
        // Draw the background.
        super.draw(c)
    }
    ...
    private fun getThumbScrollRange(): Int {
        return if (mTrackDrawable != null) {
            ...
            (mSwitchWidth - mThumbWidth - padding.left - padding.right
    ...
    protected fun onProvideStructure(
        structure: ViewStructure, viewFor: Int, flags: Int
    ) {
        val switchText = if (isChecked) mTextOn!! else mTextOff!!
        if (!TextUtils.isEmpty(switchText)) {
            val oldText = structure.text
            if (TextUtils.isEmpty(oldText)) {
                structure.text = switchText
            } else {
                val newText = StringBuilder()
                newText.append(oldText).append(' ').append(switchText)
                structure.text = newText
            }
    ...
    private val THUMB_POS: FloatProperty<XSwitchHorizontal> = object : FloatProperty<XSwitchHorizontal>("thumbPos") {
        override fun get(obj: XSwitchHorizontal): Float {
            return obj.mThumbPosition
        }

        override fun setValue(obj: XSwitchHorizontal, value: Float) {
